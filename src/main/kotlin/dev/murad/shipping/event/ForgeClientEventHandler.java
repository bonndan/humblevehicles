...
public class ForgeClientEventHandler {
    ...
    private static boolean renderRouteOnStack(RenderLevelStageEvent event, Player player, ItemStack stack) {
        ...
        if (stack.getItem().equals(ModItems.LOCO_ROUTE.get())) {
            var buffer = getBufferSource();
            var pose = new PoseStack();
            pose.mulPose(event.getProjectionMatrix());
            ...
            for (var node : LocoRouteItem.getRoute(stack)) {
                ...
                pose.translate((float) (block.getX() - cameraOff.x), (float) (1 - cameraOff.y), (float) (block.getZ() - cameraOff.z));
                pose.popPose();


                BeaconRenderer.renderBeaconBeam(pose, buffer, BEAM_LOCATION, event.getPartialTick().getGameTimeDeltaTicks(),
            ...
            var renderTypeBuffer = getBufferSource();
            ...
            for (int i = 0, routeSize = route.size(); i < routeSize; i++) {
                ...
                matrixStack.mulPose(event.getProjectionMatrix()); //???
                matrixStack.pushPose();
                {
                    matrixStack.translate(node.getX() - camPos.x, 0, node.getZ() - camPos.z);

                    BeaconRenderer.renderBeaconBeam(matrixStack, renderTypeBuffer, BEAM_LOCATION, event.getPartialTick().getGameTimeDeltaTicks(),
    ...
    private static MultiBufferSource.@NotNull BufferSource getBufferSource() {
        return MultiBufferSource.immediate(new ByteBufferBuilder(1536));
    }
    ...
    public static void onRenderWorldLast(RenderLevelStageEvent event) {
        ...
        if (mainStack.getItem().equals(ModItems.CONDUCTORS_WRENCH.get()) && player.level().dimension().toString().equals(VehicleTrackerPacketHandler.toRenderDimension)) {
            MultiBufferSource.BufferSource renderTypeBuffer = getBufferSource();
            var camera = Minecraft.getInstance().getEntityRenderDispatcher().camera;
            ...
            for (EntityPosition position : VehicleTrackerPacketHandler.toRender) {
                ...
                Vec3 entityPos = entity != null ? entity.getPosition(event.getPartialTick().getGameTimeDeltaTicks()) : position.pos();
