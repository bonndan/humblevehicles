...
public class LocomotiveNavigator {
    ...
    public void serverTick() {
        RailHelper.getRail(locomotive.getOnPos().above(), locomotive.level()).ifPresent(railPos -> {
            ...
            RailHelper railHelper = new RailHelper(locomotive);
            railHelper.getNext(railPos, moveDir).ifPresent(pair -> {
                ...
                if (state.getBlock() instanceof MultiShapeRail s && s.isAutomaticSwitching()) {
                    ...
                    if (choices.size() == 1) {
                        ...
                        if (!decisionCache.containsKey(nextRail)) {
                            var decision = railHelper.pickCheaperDir(choices, nextRail, RailHelper.samePositionHeuristicSet(potential), locomotive.level());
